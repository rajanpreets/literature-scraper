...
# --- Streamlit Cloud / Linux Display Hack ---
if os.name != 'nt':
    try:
        from pyvirtualdisplay import Display
        # Headless Chrome needs a virtual frame buffer (Xvfb) to render complex DOMs
...
def sanitize_filename(name: str) -> str:
    """Strips illegal characters from Windows/Linux filenames."""
    return re.sub(r'[\\/*?:"<>|]', "", str(name)).strip()

def sanitize_doi(doi_raw: str) -> str:
    """Formats DOI perfectly for the Unpaywall REST API."""
...
class ScraperEngine:
    # Notice we accept max_workers here so your app.py doesn't crash!
    def __init__(self, excel_path, log_callback, progress_callback, max_workers=1, paywall_wait_seconds=15):
        self.excel_path = excel_path
        self.log_callback = log_callback
        self.progress_callback = progress_callback
        self.max_workers = 1 # Internally forced to 1 to prevent cloud memory limits from being exceeded
        ...
        # Setup DataFrame for Live Tracking
    ...
    def validate_pdf(self, pdf_path, article_name="", is_conference=False):
        """Strictly validates if a downloaded file is a real PDF and not a paywall login page."""
        if not os.path.exists(pdf_path): return False
        try:
            size_kb = os.path.getsize(pdf_path) / 1024
            
            # Conference abstracts generated by our script are small; let them pass
            if is_conference: return size_kb > 1
            
            # Real full-text PDFs are rarely under 30KB
            if size_kb < 30: 
            ...
            # Pure login/paywall pages are usually exactly 1 page
            if pages <= 1:
    ...
    def _build_cloud_driver(self):
        """Builds a headless Chrome browser optimized for Streamlit Linux containers."""
        try:
            driver = Driver(
                browser="chrome",
                uc=True,            # Undetected Chromedriver - essential for bypassing Cloudflare
                headless=True,      # Streamlit has no display
                no_sandbox=True,    # Required for Docker/Linux containers
                chromium_arg="--disable-dev-shm-usage", # Fixes low-memory crashes on cloud servers
    ...
    def run(self):
        ...
        try:
            ...
            for index, row in self.df.iterrows():
                ...
                # Memory Management: Restart Chrome every 15 articles to clear Streamlit RAM
                if index % 15 == 0 and index > 0:
                    try: driver.quit()
                    except: pass
                    driver = self._build_cloud_driver()
                    
            # CRITICAL FIX: Save the finalized tracking Excel and store the path for the frontend
            self.tracked_excel_path = os.path.join(self.output_dir, "Tracked_" + sanitize_filename(os.path.basename(str(self.excel_path))))
            
            # Ensure it ends with .xlsx
            if not self.tracked_excel_path.endswith('.xlsx'):
                self.tracked_excel_path += '.xlsx'
    ...
    def process_row_autonomous(self, driver, row, duplicate_dois):
        ...
        try:
            # Use UC Reconnect to handle Cloudflare JS challenges automatically
    ...
    def annihilate_overlays(self, driver):
        """Silently clicks away cookie banners and popups."""
        js = """
        try {
    ...
    def auto_find_and_download(self, driver, format_name):
        """Hunts for PDF download links natively in the DOM."""
        candidate_urls = []
        try:
    ...
    def route_paywall_api(self, driver, doi, format_name, article_name):
        """Queries the Unpaywall REST API for legal open access PDFs."""
        if not doi: return False
        with requests.Session() as session:
            session.headers.update({"User-Agent": "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36"})
            
            # Forward cookies so institutional proxy sessions carry over
            try:
